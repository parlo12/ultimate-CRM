            ...
            <div class="d-flex align-items-end mt-75 ms-1">
                <div>
                    @if($customer->customer->activeSubscription())
                        <h5 class="mb-1 text-uppercase">{{__('locale.labels.sms_credit')}}
                            : {{ $customer->sms_unit == '-1' ? __('locale.labels.unlimited') : \App\Library\Tool::number_with_delimiter($customer->sms_unit) }}</h5>
                    @else
